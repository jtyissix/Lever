"""Build synthetic per-user train/test splits for RobustQA-style query sets."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build synthetic user splits for RobustQA")
...
def load_domain_questions(jsonl_root: Path, domain: str) -> dict[int, dict]:
    records: dict[int, dict] = {}
    idx = 1
    if domain != "finance":
        for split in ["dev", "test"]:
            path = jsonl_root / domain / split / "annotations.jsonl"
            if not path.exists():
                continue
            with path.open("r", encoding="utf-8") as f:
                for line in f:
                    records[idx] = json.loads(line.strip())
                    idx += 1
    else:
        path = jsonl_root / domain / "annotations.jsonl"
        with path.open("r", encoding="utf-8") as f:
            for line in f:
                records[idx] = json.loads(line.strip())
                idx += 1
...
def main() -> None:
    ...
    for domain in args.domains:
        domain_data[domain] = load_domain_questions(args.jsonl_root, domain)
