"""Build synthetic per-user train/test splits for MS MARCO-style query sets."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build synthetic user splits for MS MARCO")
...
def main() -> None:
    ...
    for domain in args.domains:
        domain_data[domain] = {}
        with (args.json_root / f"squad.{domain}.train.json").open("r", encoding="utf-8") as f:
            obj = json.load(f)
        idx = 1
        for item in obj["data"]:
            qa = item["paragraphs"][0]["qas"][0]
            domain_data[domain][idx] = {"question": qa["question"], "answer": qa["answers"][0]["text"]}
            idx += 1
