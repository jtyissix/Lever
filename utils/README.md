# [KDD 2026] Lever: Accelerating Graph-Based RAG Retrieval via Locality-Aware Device–Cloud Collaboration
...
3. **Generate tags with vLLM**
   - This step runs semantic tagging on the cloud index and saves the result to `RAG_TAG_CACHE_PATH`. You have to transform the long tail tags to 'Others'. You also have to transform it to addition id-domain name pkl.
   - Example:
...
python Edge_main.py sample-num-calc-2 \
  ...
  --id-domain-map "id_domain_map.pkl" \
  --query-vectors "user_1_ask.npy"
```
